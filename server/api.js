...
const publicUser = ({
  ...
  phone,
  role,
}) => ({ id, firstName, lastName, nickname, email, phone, role });
const fail = (status, messageRu, messageEn, extra = {}) => {
  throw { status, messageRu, messageEn, ...extra };
};
    ...
    try {
      ...
      if (!["GET", "HEAD"].includes(method)) {
        ...
        if (
          ...
          fail(415, "Ожидается JSON.", "JSON is required.");
      }
      let body = {};
      if (["POST", "PUT", "PATCH"].includes(method)) {
        const raw = await request.text();
        if (raw.length > 16000)
          fail(413, "Запрос слишком большой.", "Request is too large.");
        try {
          body = raw ? JSON.parse(raw) : {};
        } catch {
          fail(400, "Некорректный JSON.", "Invalid JSON.");
        }
        if (!body || typeof body !== "object" || Array.isArray(body))
          fail(400, "Ожидается объект.", "An object is required.");
      }
      const rawToken = request.headers
        .get("cookie")
        ?.match(/(?:^|;\s*)roof_session=([a-f0-9]{64})(?:;|$)/)?.[1];
      const session = rawToken
        ? await store.get("sessions", await digest(rawToken))
        : null;
      const user =
        session && session.expiresAt > Date.now()
          ? await store.get("users", session.userId)
          : null;
      const cookie = (value, age = 28800) =>
        `roof_session=${value}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${age}${url.protocol === "https:" ? "; Secure" : ""}`;
      async function signIn(account, status = 200) {
        if (session) await store.remove("sessions", session.id);
        const secret = token();
        await store.insert("sessions", {
          id: await digest(secret),
          userId: account.id,
      ...
      if (path === "/register" && method === "POST") {
        ...
        const account = {
          ...
          patronymic: clean(body.patronymic, 60),
          phone: normalizePhone(body.phone),
          email: clean(body.email, 254).toLowerCase(),
          birthDate: body.birthDate,
          nickname: body.nickname,
          role: "user",
          agreementAt: new Date().toISOString(),
          passwordHash: await hashPassword(body.password),
          createdAt: new Date().toISOString(),
        };
        try {
          await store.insert("users", account);
        } catch (e) {
          if (e.code === "CONFLICT" || /UNIQUE/.test(e.message))
            fail(
              409,
              "Email или никнейм уже занят.",
              "Email or nickname is already in use.",
            );
          throw e;
        }
      ...
      if (path === "/login" && method === "POST") {
        ...
        const account = (await store.all("users")).find(
          (u) =>
            u.email.toLowerCase() === login ||
            u.nickname.toLowerCase() === login,
        );
        const dummy = "pbkdf2:100000:" + "0".repeat(64) + ":" + "0".repeat(64);
        const valid = await verifyPassword(
          clean(body.password, 100),
      ...
      if (path === "/projects" && method === "GET") {
        ...
        rows.sort(
          sort === "title"
            ...
            : sort === "oldest"
              ? (a, b) => a.dateTo.localeCompare(b.dateTo)
              : (a, b) => b.dateTo.localeCompare(a.dateTo),
        );
        ...
        return json({
          items: rows.slice((page - 1) * 6, page * 6),
          total,
          pages,
          page,
        });
      ...
      if (path === "/favorites" && method === "GET") {
        if (!user) denied();
        const rows = (await store.all("favorites")).filter(
          (f) => f.userId === user.id,
        );
      ...
      if (
        ...
        if (!user) denied();
        const projectId = decodeURIComponent(path.slice(11)),
          id = `${user.id}:${projectId}`;
        if (!(await store.get("projects", projectId)))
          fail(404, "Объект не найден.", "Project not found.");
        if (method === "PUT")
          await store.put("favorites", { id, userId: user.id, projectId });
      ...
      if (path === "/requests" && method === "POST") {
        ...
        if (
          !["question", "partner"].includes(body.type) ||
          !clean(body.name, 120) ||
          !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(clean(body.email, 254)) ||
          typeof body.message !== "string" ||
          body.message.trim().length < 10 ||
          body.message.length > 2000 ||
          ![true, "on"].includes(body.consent) ||
          (body.type === "partner" && !clean(body.company, 120))
        )
          fail(
            422,
            "Проверьте имя, email, сообщение и согласие.",
            "Check your name, email, message and consent.",
          );
        const row = {
          id: crypto.randomUUID(),
          userId: user?.id || null,
      ...
      if (path === "/requests" && method === "GET") {
        if (!user) denied();
        let rows = (await store.all("requests")).filter(
          (r) => user.role === "admin" || r.userId === user.id,
        );
      ...
      if (path.startsWith("/requests/") && method === "PATCH") {
        ...
        if (!statuses.has(body.status))
          fail(422, "Неизвестный статус.", "Unknown status.");
      ...
      return json(
        { messageRu: "Ресурс не найден.", messageEn: "Resource not found." },
        404,
      );
    } catch (e) {
      if (e.status)
      ...
      return json(
        {
          messageRu: "Ошибка сервера. Попробуйте позже.",
