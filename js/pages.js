...
export function agreement() {
  return `${header()}<main id="main" class="form-page agreement-page"><h1>${tr("Пользовательское соглашение", "User agreement")}</h1><div class="prose">${(preferences.language === "ru" ? agreementRU : agreementEN).map((p) => `<p>${p}</p>`).join("")}</div><p style="margin-top:32px">${routeLink("register", "Перейти к регистрации", "Continue to registration", "button")}</p></main>${footer()}`;
}
